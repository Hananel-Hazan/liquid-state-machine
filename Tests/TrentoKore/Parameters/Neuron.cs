...
namespace Liquid_Detector.Tests.TrentoKore.Parameters
{
	...
	/// </summary>
	[Serializable()]
	public class Neuron4TrentoKore: NeuronParametes
	{
		//----------
		
		public Neuron4TrentoKore()
		{
			...
			STDPwindow = new int[]{5,2};// past,future ==> pre , post ==> LTP , LTD
			STDPMaxChange = new double[]{0.01,0.01}; // past,future // pre , post
			Active_STDP_rule = 1 ; // 0 no 1 yes
			...
			if (this.Neuron_Model==1){ // Normal LIF
				this.decayFactor = 0.4 ; //   0.99 = 99% decay 0 = 0% decay
				...
				this.Proportional_Threshold = 1  ;  // proportional? 0 = no , 1 = yes
		...
		override public void initNueronMaxParameters(int one_Second){
			...
			this.Neuron_Slideing_Threshold_Recommended_Firing_Rate_Max = 0.95 * this.Neuron_Firing_Rate_Max;
			this.Neuron_Slideing_Threshold_Recommended_Firing_Rate_Min = 0.001 * this.Neuron_Firing_Rate_Max;
			this.Neuron_STDP_Recommended_Firing_Rate_Max = 0.8 * this.Neuron_Firing_Rate_Max;
		...
		public Neuron4TrentoKore copy(){
			Neuron4TrentoKore source = (Neuron4TrentoKore) this.MemberwiseClone();
			return(source);
		}//----------------------------------------------------------------------------------
		...
		public Neuron4TrentoKore load(string args){
			Stream fileStream = new FileStream(args, FileMode.Open,FileAccess.Read, FileShare.None);
			BinaryFormatter binaryFormater = new BinaryFormatter();
			Neuron4TrentoKore Param = (Neuron4TrentoKore) binaryFormater.Deserialize(fileStream);
			fileStream.Close();
			return Param;
		}//----------------------------------------------------------------------------------
