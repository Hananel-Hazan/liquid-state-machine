	...
	internal class Procedures
	{
		...
		public static Model svm_train(Problem prob, Parameter param)
		{
			    ...
			    param.SvmType == SvmType.NU_SVR)
			{
				...
				//                model.SupportVectorCoefficients = new double[1,1];
				...
				model.SupportVectorCoefficients = new double[1,nSV];
			...
			else
			{
				...
				model.SupportVectorCoefficients = new double[nr_class - 1,nnz];
		...
		public static void svm_predict_values(Model model, Node[] x, double[] dec_values)
		{
			    ...
			    model.Parameter.SvmType == SvmType.NU_SVR)
			{
				...
				for (int i = 0; i < model.SupportVectorCount; i++)
					sum += model.SupportVectorCoefficients[0,i] * Kernel.KernelFunction(x, model.SupportVectors[i], model.Parameter);
				sum -= model.Rho[0];
				dec_values[0] = sum;
			}
			else
...
//					double[] coef2 = model.SupportVectorCoefficients[i];
					for (k = 0; k < ci; k++)
						sum +=  model.SupportVectorCoefficients[temp,si + k] * kvalue[si + k];
//						sum += coef1[si + k] * kvalue[si + k];
					for (k = 0; k < cj; k++)
						sum += model.SupportVectorCoefficients[i,sj + k] * kvalue[sj + k];
