...
namespace Liquid_Detector.Tests.TrentoKore.Parameters
{
	...
	/// </summary>
	[Serializable()]
	public class RecurrentNetwork4TrentoKore : NetworkParm
	{
		public RecurrentNetwork4TrentoKore(int Neuron_Model, ref globalParam Param)
		{
			/* 
		 	0  = random ,
			1  = Old Mathod,
			...
			this.Neuron_propotional_weight_Update = 0; // 0=Normal update weights min/max weight is valid,1=initilize and learn will be propotoinal if one increase other will decreas
			
			//----------------Neuron Modle-------------------------------------
			if (Neuron_Model==1){ // Normal LIF
				this.LSM_Max_Init_Weight_NegN = 0.0; //1.01;
				this.LSM_Min_Init_Weight_NegN = 0.0; //1;
				this.LSM_Max_Init_Weight_PosN = 0.0; //1.01;
				this.LSM_Min_Init_Weight_PosN = 0.0; //1;
			}
		...
		public RecurrentNetwork4TrentoKore copy(){
			RecurrentNetwork4TrentoKore source = (RecurrentNetwork4TrentoKore) this.MemberwiseClone();
			return(source);
		}//----------------------------------------------------------------------------------
		...
		public RecurrentNetwork4TrentoKore load(string args){
			Stream fileStream = new FileStream(args, FileMode.Open,FileAccess.Read, FileShare.None);
			BinaryFormatter binaryFormater = new BinaryFormatter();
			RecurrentNetwork4TrentoKore Param = (RecurrentNetwork4TrentoKore) binaryFormater.Deserialize(fileStream);
			fileStream.Close();
			return Param;
		}//----------------------------------------------------------------------------------
