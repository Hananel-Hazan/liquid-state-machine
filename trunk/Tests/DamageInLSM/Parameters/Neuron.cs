...
namespace Liquid_Detector.Tests.DamageLSM.Parameters
{
	...
	/// </summary>
	[Serializable()]
	public class Neuron4DamageLSM: NeuronParametes
	{
		//----------
		
		public Neuron4DamageLSM()
		{
			...
			STDPwindow = new int[]{5,5};// past,future ==> pre , post
			STDPMaxChange = new double[]{0.001,0.001}; // past,future // pre , post
			...
			Random_Factor_Sliding_Treshold = 0.01; // each neuron have random element that this factor multiplay it
			Randomize_initilaization = 1; // upon rest the neuron will randomize the Volt and Threshold
		...
		override public void initNueronMaxParameters(int one_Second){
			...
			this.Neuron_Slideing_Threshold_Recommended_Firing_Rate_Max = 0.5 * this.Neuron_Firing_Rate_Max;
		...
		public Neuron4DamageLSM copy(){
			Neuron4DamageLSM source = (Neuron4DamageLSM) this.MemberwiseClone();
			return(source);
		}//----------------------------------------------------------------------------------
		...
		public Neuron4DamageLSM load(string args){
			Stream fileStream = new FileStream(args, FileMode.Open,FileAccess.Read, FileShare.None);
			BinaryFormatter binaryFormater = new BinaryFormatter();
			Neuron4DamageLSM Param = (Neuron4DamageLSM) binaryFormater.Deserialize(fileStream);
			fileStream.Close();
			return Param;
		}//----------------------------------------------------------------------------------
