...
namespace Liquid_Detector.Tests.DamageLSM.Parameters
{
	...
	/// </summary>
	[Serializable()]
	public class ReadOut4DamageLSM : Readout
	{
		public ReadOut4DamageLSM (){
			/*
		 MODEL
		 1 = Perceptron
		 2 =  === CANCEL===ADELINE===
		...
		public ReadOut4DamageLSM copy(){
			ReadOut4DamageLSM source = (ReadOut4DamageLSM) this.MemberwiseClone();
			return(source);
		}//----------------------------------------------------------------------------------
		...
		public ReadOut4DamageLSM load(string args){
			Stream fileStream = new FileStream(args, FileMode.Open,FileAccess.Read, FileShare.None);
			BinaryFormatter binaryFormater = new BinaryFormatter();
			ReadOut4DamageLSM Param = (ReadOut4DamageLSM) binaryFormater.Deserialize(fileStream);
			fileStream.Close();
			return Param;
		}//----------------------------------------------------------------------------------
