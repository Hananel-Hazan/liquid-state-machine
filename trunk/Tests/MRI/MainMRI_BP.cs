...
namespace Liquid_Detector.Tests.MRI_BP
{
	...
	class MRIProgram_BP
	{
		...
		public static void MainProcess(string args)
		{
			...
			for (int iterationNumber=0 ; iterationNumber < Param.test.NumOfVoxels; iterationNumber++){
				...
				return_Targets(Param.LearnData,out targetLearnData);
				
				// init the Detector
				ReadOut_Detector NNet = new ReadOut_Detector(Param.LearnData[0].Input.Length,2,1,1,ref Param);
				// finish the Detector
				
				double[] LastReturnError= new double[0];
//				NNet.StratTrain(ref Param,out LastReturnError);

				// testing Detector
				Console.WriteLine("");
				Console.WriteLine("Start Normal Testing Vectors");
				double[] DetectorOutput = new double[0];
				double[] targetTestData;
				return_Targets(Param.TestData,out targetTestData);
//				NNet.StratTesting(ref Param,out DetectorOutput);

				Matrix_Arithmetic MatrixTemp = new Matrix_Arithmetic();
				
				tw.WriteLine("Detector Last Treaining error = {0}",MatrixTemp.Vector_Average(LastReturnError));
				for(int i=0 ; i<DetectorOutput.Length ; i++){
					tw.WriteLine("Vector#\t{0}, Target\t= {1}, Detector degree of correction\t = {2}",Param.CurrentGroup,targetLearnData[i],DetectorOutput[i]);
				}
		...
		[STAThread]
		public static void MainMRI_BP(string[] args)
		{
			if (args.Length>0){
				MainProcess(args[0]);
			}
