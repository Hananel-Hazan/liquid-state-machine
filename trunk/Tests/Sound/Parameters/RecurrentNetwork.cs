...
namespace Liquid_Detector.Tests.Sound.Parameters
{
	...
	/// </summary>
	[Serializable()]
	public class RecurrentNetwork4Sound : NetworkParm
	{
		public RecurrentNetwork4Sound(int Neuron_Model, ref globalParam Param)
		{
			/* 
		 	0  = random ,
			1  = Old Mathod,
			...
			Liquid_Readout_Units = 1 ;  // 0 = random, 1 = minimum input/output but bigger then zero, 2=maximum,3=middle
			...
			IncreaseSelectedChanceBy = 10 ;
			Number_Of_Neurons = 45 ;
			...
			LSM_Input_Percent_Connectivity = 0.4 ; // Maass Parameter is 30%
			Liquid_Weights_Resistend = 1.0; // 1.1 = 10 % every delay interval
			this.Neuron_propotional_weight_Update = 1; // 0=min/max weight is valid,1=initilize and learn will be propotoinal if one increase other will decreas
		...
		public RecurrentNetwork4Sound copy(){
			RecurrentNetwork4Sound source = (RecurrentNetwork4Sound) this.MemberwiseClone();
			return(source);
		}//----------------------------------------------------------------------------------
		...
		public RecurrentNetwork4Sound load(string args){
			Stream fileStream = new FileStream(args, FileMode.Open,FileAccess.Read, FileShare.None);
			BinaryFormatter binaryFormater = new BinaryFormatter();
			RecurrentNetwork4Sound Param = (RecurrentNetwork4Sound) binaryFormater.Deserialize(fileStream);
			fileStream.Close();
			return Param;
		}//----------------------------------------------------------------------------------
